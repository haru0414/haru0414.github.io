...
const zh = {
  translation: {
    ...
    projects: {
      ...
      "10": {
        desc: "驗屋現場用的 React Native / Expo App，Offline-First 架構：畫面優先讀本地 SQLite 與離線圖包，網路只負責背景推送與增量同步",
        problem: "驗屋作業發生在剛完工的建案裡，地下室與樓梯間常常沒有訊號。一個案件動輒數百個檢測項目與照片，靠即時連線儲存，一斷線就整筆遺失。",
        approach: "與另一名前端工程師協作。採 Offline-First 架構，所有畫面優先讀本地 SQLite 與離線圖包，網路只負責背景推送與增量同步。設計待送操作與待送照片兩條佇列，以 AppState 喚醒、10 分鐘定期輪詢與下拉重整三種觸發推送，網路失敗指數退避至最長 30 分鐘；儲存改為批次送出（停止操作 60 秒觸發，累積 10 筆操作或 3 張照片提前送），降低反覆喚醒行動網路的耗電與發熱。以未上傳項目清單作為拉取的保險絲，避免 server 舊值覆蓋本地尚未同步的編輯。案件明細走版本化同步，離線圖包支援增量下載、取消與孤兒回收。報告以 react-native-pdf 產出，錯誤與行為追蹤接 Firebase Crashlytics / Analytics。",
        result: "2025 年 7 月起持續開發至今，版本迭代至 2.3.8，經 EAS Build 發佈 iOS 與 Android。",
      },
      "11": {
        desc: "K-pop 追星地圖 App：活動探索、地圖找地點、行程規劃與數位票券收藏，Expo SDK 57 + Expo Router + NativeWind",
        problem: "追星的資訊散落在各個社群平台——生日應援辦在哪、快閃店開到幾號、周邊在哪個點領取，每次都要重新翻一輪。",
        approach: "團隊專案，我負責前端開發；另一位前端夥伴負責 flow chart、UI/UX 與 design system，後端由團隊其他成員負責。以 Expo SDK 57 + Expo Router 檔案式路由建構，樣式採 NativeWind，動畫用 Reanimated。地圖以 react-native-maps 實作城市／區域切換與六類地點標記篩選（場館、應援活動、應援物領取點、快閃店、周邊販售點、打卡景點）。活動、地點與票券共用單一 Context 全域狀態，避免跨頁資料不同步。功能面完成活動探索、行程倒數與今日行程、收藏與到訪記錄，以及可新增／編輯／刪除的數位票券收藏。測試以 Jest + React Native Testing Library 覆蓋。",
        result: "2026 年 7 月啟動，持續開發中。",
        shots: {
          "0": {
            label: "活動探索",
            desc: "首頁輪播主打活動，下方以熱門／近期兩個分頁列出活動，可直接收藏或加入行程。",
          },
          "1": {
            label: "活動詳情",
            desc: "場次時間、場館地址與售票資訊集中在同一張卡，並帶出該活動的相關地點（場館、應援站、打卡牆）。",
          },
          "2": {
            label: "我的行程",
            desc: "已加入的活動以倒數卡呈現，月曆標出有行程的日期，下方接當日的時間軸。",
          },
          "3": {
            label: "數位票券",
            desc: "以票根樣式收藏看過的場次，上方統計即將到來、今年場次與票券總數，票券可新增、編輯與刪除。",
          },
          "4": {
            label: "個人檔案",
            desc: "收藏與到訪的統計、票券入口、幫助中心與系統設定。",
          },
        },
      },
      "12": {
        desc: "追星地圖 App 的管理後台：管理員帳號與細粒度權限、活動與地點管理、應援申請審核、法務文件版本控管",
        problem: "App 端的活動、地點、票券與應援申請都需要有人維護，且後台的資料模型必須與 App 完全一致，否則兩邊會各長出一套格式。",
